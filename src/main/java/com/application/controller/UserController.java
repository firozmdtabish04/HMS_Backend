...
public class UserController {
	...
	public ResponseEntity<Appointments> addNewAppointment(
		...
		for (Slots slot : availableSlots) {
			if (slot.getDoctorname().equals(appointment.getDoctorname())
		...
		// If no slot found → STOP
		if (selectedSlot == null) {
			throw new Exception(
					"Doctor has no slots available on selected date.");
		}

		String message = "Slot already booked. Please select another slot.";

		// Check AM slot
		if (appointment.getSlot().equalsIgnoreCase("AM slot")
				&& selectedSlot.getAmstatus().equalsIgnoreCase("booked")) {
			throw new Exception(message);
		}

		// Check PM slot
		if (appointment.getSlot().equalsIgnoreCase("PM slot")
				&& selectedSlot.getPmstatus().equalsIgnoreCase("booked")) {
			throw new Exception(message);
		}

		// Check Noon slot
		if (appointment.getSlot().equalsIgnoreCase("Noon slot")
				&& selectedSlot.getNoonstatus().equalsIgnoreCase("booked")) {
			throw new Exception(message);
		}

		// Save appointment
		Appointments savedAppointment = appointmentBookingService.addNewAppointment(appointment);

		// Generate Patient ID
		String patientID = getPatientID();

		appointmentBookingService.updatePatientId(
				patientID,
		...
		if (appointment.getSlot().equalsIgnoreCase("PM slot")) {
			appointmentBookingService.bookPMSlot(
					appointment.getDoctorname(),
					appointment.getDate());
		}

		if (appointment.getSlot().equalsIgnoreCase("AM slot")) {
			appointmentBookingService.bookAMSlot(
					appointment.getDoctorname(),
					appointment.getDate());
		}

		if (appointment.getSlot().equalsIgnoreCase("Noon slot")) {
			appointmentBookingService.bookNoonSlot(
					appointment.getDoctorname(),
					appointment.getDate());
		}
